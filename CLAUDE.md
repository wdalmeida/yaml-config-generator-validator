...
A static site (no backend) that helps users fill out and validate the YAML config file our software requires in its repository, so they stop hand-editing it incorrectly. Two modes: **Generate** (optionally load an existing config from GitHub or paste it in, edit the form, get valid YAML → copy/paste or push to GitHub) and **Validate** (paste YAML → schema errors).
...
- `src/lib/github.ts` — the entire GitHub integration, intentionally with no OAuth, no GitHub App, no token handling; every call is an unauthenticated request to GitHub's public API, so all of this only works reliably for public repos.
...
- **Load existing config** (`GeneratorForm`'s "Load existing config" section): lets the user hydrate the whole form from a config that's already committed, instead of retyping it before generating an update. Two paths, both funnel through `parseYamlConfig` (same schema as everything else) into a shared `applyConfig(config)` that sets every field: "Load from GitHub" calls `fetchFileContent` against the owner/repo/branch/path fields (fails closed with a message pointing at the paste option for private repos); "Load pasted YAML" parses whatever's typed into the adjacent textarea. `applyConfig` re-derives `tenantMode` by checking membership in `EXISTING_TENANTS`, so a loaded config with an unlisted tenant name correctly lands in "new tenant" rather than silently not matching any `<option>`.
