...
`.size-limit.json` sets a size budget on the built output (`dist/assets/*.js` / `*.css`, gzipped) checked by `lint:size` - current build is ~113 KB JS / ~1 KB CSS gzipped, limits are 150 KB / 5 KB (headroom, not a tight budget). Bump the limits deliberately if a real feature needs it; don't just raise them to make a failing check pass without checking why the bundle grew.
