...
- `release.yml` — automated versioning/changelog/tag/GitHub Release via release-please (Conventional Commits required from adoption forward - see `docs/releasing.md`), plus a `build-and-attach` job that zips `dist/` onto the release once release-please's standing PR is actually merged. A repository ruleset (not a workflow - a live repo setting, see `docs/releasing.md`) makes `v*` tags immutable (update+deletion blocked, admin bypass only) once created.
