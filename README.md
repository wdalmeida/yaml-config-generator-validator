...
Each config type is one two-column screen: the left column has target file location, that
type's own fields, and Push to GitHub, scrolling normally as the form grows; the right column
is one live, editable YAML field, pinned in view as you scroll the form so it's never out of
sight — fill in the form and watch it update, or fetch/paste/edit YAML directly and watch the
form sync back once it's valid. Push it as a new file via a prefilled "create file" link, or
update an existing one (copies the YAML for you, since GitHub can't prefill an edit).
