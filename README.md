...
Each config type is one two-column screen: the left column (target file location, Push to
GitHub) is the same across every type; the right column has that type's own fields plus one
live, editable YAML field — fill in the form and watch it update, or fetch/paste/edit YAML
directly and watch the form sync back once it's valid. Push it as a new file via a prefilled
"create file" link, or update an existing one (copies the YAML for you, since GitHub can't
prefill an edit).
